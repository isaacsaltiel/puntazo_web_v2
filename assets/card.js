...
// Deps: PuntazoFirebase (requerido), PuntazoReactions (opcional),
//       PuntazoAuth (opcional), Firebase compat SDK v9
// =============================================================
window.PuntazoCard = (function () {
  ...
  function toast(msg) {
    ...
    if (!el) {
      ...
      el.style.cssText = 'position:fixed;left:50%;bottom:26px;transform:translateX(-50%);background:rgba(0,0,0,.86);color:#fff;padding:10px 18px;border-radius:10px;z-index:9999;font-weight:600;font-size:14px;pointer-events:none;white-space:nowrap;';
  ...
  // ── Action pill factories ──────────────────────────────────
  function makePill(text, title, extra) {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'action-pill' + (extra?' '+extra:'');
  ...
  function buildSavePill(entry) {
    ...
    const sync = async ()=>{
      ...
      try {
        ...
        btn.title = saved?'Guardado en tu perfil':'Guardar en tu perfil';
      } catch {}
    };
    ...
    btn.addEventListener('click', async ()=>{
      ...
      try {
        ...
        if (alreadySaved) { await unsaveVideo(entry.nombre); toast('Quitado de guardados'); }
        else { await saveVideo(entry); toast('¡Guardado en tu perfil!'); }
  ...
  function buildRemovePill(onRemove) {
    const btn = makePill('✕','Quitar de guardados','remove-pill');
    btn.addEventListener('click', ()=>{ if(typeof onRemove==='function') onRemove(); });
    return btn;
  }

  function buildFullscreenPill(video, card) {
    bindFsEvents();
    ...
    btn.addEventListener('click', async ()=>{
      try {
        ...
        else { if(video.paused){try{await video.play();}catch{}} await requestFs(video); try{if(screen.orientation?.lock)await screen.orientation.lock('landscape');}catch{} }
        syncAll();
   ...
   *
   * entry: { nombre, url, club?, cancha?, lado?, fecha?, _meta? }
   * opts:  {
   *   showSave:     true,
   *   showShare:    true,
   *   showFullscreen: true,
   ...
   *   showReactions: true,
   *   showComments: true,
   *   showClubInfo: false,   // show club+cancha subtitle
   *   rankBadge:    null,    // 1|2|3 → show rank badge
   *   onRemove:     null,    // fn() → show ✕ pill
   * }
   *
   * IMPORTANTE: llama attachReactions(card, entry) DESPUÉS de insertar en DOM.
   */
  function build(entry, opts) {
    opts = Object.assign({
      ...
      showClubInfo:false, rankBadge:null, onRemove:null,
    }, opts||{});
    ...
    // Subtítulo opcional (club/cancha)
    if (opts.showClubInfo && (entry.club || entry.cancha)) {
      const sub = document.createElement('div');
      sub.className = 'card-subtitle';
      sub.textContent = [entry.club, entry.cancha ? 'Cancha '+entry.cancha : ''].filter(Boolean).join(' · ');
    ...
    if (opts.onRemove)        pillsEl.appendChild(buildRemovePill(opts.onRemove));
    if (opts.showShare)       pillsEl.appendChild(buildSharePill(entry));
    if (opts.showSave)        pillsEl.appendChild(buildSavePill(entry));
    if (opts.showFullscreen)  pillsEl.appendChild(buildFullscreenPill(video, card));
  ...
  // Llama DESPUÉS de insertar la card en el DOM
  function attachReactions(card, entry) {
    if (!opts_hasReactions(card)) return;
  ...
  function opts_hasReactions(card) {
    return !!(card.querySelector('[data-rxn-slot],[data-comments-slot],[data-claim-slot]'));
  }

  // Shorthand: build + append + attachReactions
  ...
  async function loadEntryFromConfig(videoId) {
    ...
    if (!found) return null; // video expiró
